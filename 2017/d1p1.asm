...
section '.const' data readable
    t1 db 1,1,2,2           ; produces a sum of 3 (1 + 2) because the first digit (1) matches the second digit and the third digit (2) matches the fourth digit.
    len.t1 = $ - nums

    t2 db 1,1,1,1           ; produces 4 because each digit (all 1) matches the next.
    len.t2 = $ - nums

    t3 db 1,2,3,4           ; produces 0 because no digit matches the next.
    len.t3 = $ - nums

    t4 db 9,1,2,1,2,1,2,9   ; produces 9 because the only digit that matches the next one is the last digit, 9.
    len.t4 = $ - nums

    inp db 5, 7, 2, 7, 6, 2, 7, 4, 3, 8, 7, 9, 4, 4, 5, 3, 7, 8, 2, 3, 6, 5, 2, 6, 2, 6, 1, 7, 7, 8, 5, 3, 3, 8, 4, 4, 1, 1, 1, 4, \
...
start:
    ...
    xor     edx, edx            ; previous value
    mov     esi, nums           ; array ptr
    mov     ecx, len.nums - 1   ; counter
